...
export const characterOverridesShape = z.object({
  ...
  tools: z.object({
     ...
     *  `computeEffectiveProficiencies` (pc.decision-engine.ts). A key a WRITER CREATES is
     ...
     *  `addProficiency` clearance) match an existing key by slug too, so a key the note ALREADY
     *  carries is written THROUGH in its own spelling rather than duplicated · nothing here
     *  re-spells it (review M-4: "repaired in place" read as "normalised", which it is not).
     *  `languages` gets no such sibling: languages have no tri (§9.3). */
