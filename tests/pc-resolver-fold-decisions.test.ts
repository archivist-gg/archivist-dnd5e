...
describe("a folded feature's lower copies keep their decisions at their own levels (spec §7.1 rider)", () => {
  ...
  it("the decision ledger offers the row at BOTH levels, each resolved (m11's kill row)", () => {
    ...
    expect(items.map((i) => i.level)).toEqual([1, 6]);
    ...
    expect(items.map((i) => i.selected)).toEqual([["stealth", "perception"], ["arcana", "athletics"]]);
  });
  ...
  it("the wrapper carries ONE entry per choice-CARRYING lower copy, and the top copy's choices stay on the feature", () => {
    ...
    // The level-3 copy carries no choices, so it contributes no entry: a walker visits `foldedFrom` then
    // `feature.choices`, and no copy's choices are visited twice.
    expect(rf.foldedFrom?.map((c) => c.level)).toEqual([1]);
