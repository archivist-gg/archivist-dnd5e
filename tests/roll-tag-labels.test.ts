 ...
 * `ROLL_NOUN` is `Record<RollKind, string>`, so a missing key is a COMPILE error (§14 row 8i)
...
describe("roll tag labels (R4-G3a §5.2)", () => {
  ...
  it("mode words + roll nouns", () => {
    ...
    expect(ROLL_NOUN).toEqual({ "ability-check": "ability checks", "saving-throw": "saving throws", attack: "attack rolls" });
  });
